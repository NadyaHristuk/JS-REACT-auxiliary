...
class App extends Component {
  ...
  componentDidMount() {
    window.socket.on('register-on-DB', (message, tocken) => {
    ...
    window.socket.on('login-done', (message, tocken) => {
      console.log('login-done', message);           
     
        window.socket.io.opts.query = {
          token: tocken
        }
        console.log('nowiy token',  window.socket.io);
